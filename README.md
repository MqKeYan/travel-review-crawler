...
<p align="center">
  <strong> 支持携程、飞猪、淘宝、天猫等平台，覆盖旅游景点、酒店民宿、购物网站三大分类，一键爬取评论数据 </strong>
</p>
...
| 功能 | 说明 |
|------|------|
| 🌐 多平台支持 | 覆盖旅游景点、酒店民宿、购物网站三大分类，预设携程、飞猪、淘宝、天猫适配器 |
| 🔗 URL 智能识别 | 输入链接自动识别爬取类型和目标网站，支持 ID 拼接完整 URL |
| 🍪 Cookie 管理 | 一键拉取系统浏览器（Edge / Chrome / Firefox）的登录 Cookie，按平台分类存储 |
| 🔐 验证码处理 | 检测到验证码时通知用户手动处理，通过后自动恢复爬取 |
| 🖼️ 图片下载 | 评论图片多线程下载原图到本地，DOCX/XLSX 导出时自动嵌入 |
| 📤 多格式导出 | TXT · CSV · XLSX · DOCX，支持关键字过滤、图片过滤、纯表情过滤 |
| 🔔 任务通知 | 桌面弹窗 + 声音提示 + PushPlus 微信推送 |
| 🎨 双主题 | 暗夜绿（暗色）/ 晨曦绿（亮色）/ 跟随系统，窗口标题栏同步 |
| 🌐 代理支持 | 支持 HTTP/HTTPS 代理配置与连通性测试 |
...
| 平台 | 爬取模式 |
|------|----------|
| 携程(景点) | requests（首页）+ Selenium 点击翻页（后续页） |
| 飞猪 | Selenium 滚动加载 |
| 淘宝/天猫 | Selenium 滚动加载 |
| 携程(酒店) | requests（首页）+ Selenium 翻页 |
