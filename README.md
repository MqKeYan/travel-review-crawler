...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;评价爬虫器是一款基于 PySide6 构建的 Windows 桌面应用，专注于**旅游景区的用户评论数据采集**，软件采用**暗夜绿**暗色主题与**晨曦绿**浅色主题双皮肤，支持高 DPI 自适应，适配 720P ~ 4K 分辨率，提供三栏布局的现代化桌面交互界面。软件通过纯 HTTP 请求 + Selenium 浏览器渲染两种模式，覆盖携程、飞猪等平台的评论爬取（目前去哪儿和大众点评还在开发适配中）。内置 Cookie 管理、图片本地下载、多格式导出（TXT / CSV / XLSX / DOCX）和桌面通知推送，提供开箱即用的完整体验。
